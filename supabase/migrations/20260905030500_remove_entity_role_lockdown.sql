-- P0 security: harden app_public_remove_entity role checks (Chevy/Eustekio)
-- Hard-delete of fiscal/financial entities: owner/admin only.
-- Soft-deactivate (customer/product/supplier/branch) keeps prior roles.
-- Cashier must cancel sales via app_public_cancel_sale (not hard-delete).
...
begin
  ...
  -- Soft-deactivate: keep existing role matrix
  ...
  -- Hard-delete fiscal documents: owner/admin only
  ...
  -- Hard-delete cash movements: owner/admin only (was cashier)
  ...
  -- Hard-delete purchase receipts: owner/admin only (aligned; was owner/admin/warehouse)
  ...
  -- Hard-delete sales: owner/admin only; cashier must use app_public_cancel_sale
  if v_entity = 'sale' then
    if v_role not in ('owner', 'admin') then
      raise exception 'use_cancel_sale';
    end if;
    select * into v_sale
    from public.sales
    where id = v_entity_id and commerce_id = v_ctx.session_commerce_id
