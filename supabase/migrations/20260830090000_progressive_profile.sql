-- Perfil de alta progresivo: opcional, sin impacto sobre la operación del POS.
...
begin
  ...
  if length(trim(coalesce(p_country, ''))) > 80 or length(trim(coalesce(p_industry, ''))) > 100 or length(trim(coalesce(p_phone, ''))) > 30 then
    raise exception 'progressive_profile_value_too_long';
  end if;
  if nullif(trim(coalesce(p_phone, '')), '') is not null and trim(p_phone) !~ '^[+()0-9[:space:]-]{6,30}$' then
    raise exception 'invalid_phone';
  end if;
  if v_status not in ('pending', 'complete') then
    raise exception 'invalid_onboarding_status';
  end if;
  if jsonb_typeof(v_goals) <> 'array' or jsonb_array_length(v_goals) > 5 or exists (
    select 1 from jsonb_array_elements_text(v_goals) goal
    where goal not in ('vender', 'stock', 'caja', 'clientes', 'facturacion', 'sucursales')
  ) then
    raise exception 'invalid_operational_goals';
  end if;

  update public.commerce_accounts
  set onboarding_country = trim(coalesce(p_country, '')),
      onboarding_industry = trim(coalesce(p_industry, '')),
      onboarding_phone = trim(coalesce(p_phone, '')),
      onboarding_needs_arca = p_needs_arca,
      onboarding_goals = v_goals,
...
revoke all on function public.app_public_update_progressive_profile(text, text, text, text, boolean, jsonb, text) from public, authenticated;
grant execute on function public.app_public_update_progressive_profile(text, text, text, text, boolean, jsonb, text) to anon;
