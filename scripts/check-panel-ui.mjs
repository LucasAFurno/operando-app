...
try {
  ...
  for (const width of [320, 390, 740]) {
    ...
    const suggestions = page.locator('.quick-search-suggestions [role="option"]')
    await suggestions.first().waitFor({ state: 'visible' })
    assert.ok(await suggestions.count(), 'Typing must show custom suggestions')
    await page.locator('[data-action="choose-topbar-search"][data-search-section="tickets"]').first().click()
    await page.waitForURL('**/servicios/')
    await page.getByRole('searchbox', { name: 'Buscar tickets', exact: true }).waitFor()
    await page.getByRole('button', { name: 'Resumen', exact: true }).click()
