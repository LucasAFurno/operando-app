...
const homeTabImages = [
  { id: 'ventas', label: 'Ventas', image: '/operando-punto-venta-real.png', alt: 'Pantalla de ventas de operando.app' },
  { id: 'stock', label: 'Stock', image: '/operando-stock-real.png', alt: 'Control de stock de operando.app' },
  { id: 'caja', label: 'Caja', image: '/operando-panel-real.png', alt: 'Panel de caja y operación de operando.app' },
]

const renderHomeHero = (page) => `
  <section class="home2-hero" aria-label="Presentación">
    <div class="home2-hero-copy">
      <p class="home2-kicker"><span class="home2-dot" aria-hidden="true"></span>${escapeHtml(page.kicker)}</p>
      <h1>${escapeHtml(page.h1)}</h1>
      <p class="home2-lead">${escapeHtml(page.lead)}</p>
      <div class="home2-hero-actions">
        <a class="home2-btn-primary" data-analytics="hero_start_trial" href="${page.primaryCta?.href || signupPath}">${escapeHtml(page.primaryCta?.label || 'Probar gratis')}</a>
        <a class="home2-btn-link" data-analytics="hero_login_inline" href="${loginPath}">¿Ya tenés cuenta? <span>Iniciar sesión</span></a>
      </div>
    </div>
    <div class="home2-hero-panel">
      <div class="home2-frame">
        ${homeTabImages.map((tab, index) => `<input type="radio" name="home2-tab" id="home2-tab-${tab.id}" class="home2-tab-input" ${index === 0 ? 'checked' : ''} />`).join('')}
        <div class="home2-frame-bar">
          <span class="home2-frame-dot" aria-hidden="true"></span>
          ${homeTabImages.map((tab) => `<label for="home2-tab-${tab.id}" class="home2-frame-tab">${escapeHtml(tab.label)}</label>`).join('')}
        </div>
        <div class="home2-frame-body">
          ${homeTabImages.map((tab, index) => `<div class="home2-pane" data-pane="${tab.id}"><img src="${tab.image}" alt="${escapeHtml(tab.alt)}" width="1200" height="800" loading="${index === 0 ? 'eager' : 'lazy'}" ${index === 0 ? 'fetchpriority="high"' : ''} /></div>`).join('')}
        </div>
      </div>
      ${(page.stats || []).length ? `
      <div class="home2-statusbar">
        ${(page.stats || []).map(([label, value]) => `<div class="home2-status-item"><b>${escapeHtml(label)}</b><span>${escapeHtml(value)}</span></div>`).join('')}
      </div>` : ''}
    </div>
  </section>
`
...
const renderHomeExtras = (page) => {
  ...
  const verticals = [...(marketingMetrics.verticals || []), 'tu rubro']
  return `
  <section id="como-funciona" class="home2-flow" aria-labelledby="flow-title">
    <div class="home2-section-head">
      <p class="home2-kicker-plain">Cómo se usa en el día a día</p>
      <h2 id="flow-title">Del mostrador al depósito, en una sola operación</h2>
    </div>
    <div class="home2-flow-track">
      ${homeFeatureRows.map((row, index) => `
        <article class="home2-flow-step">
          <div class="home2-flow-num">${String(index + 1).padStart(2, '0')}</div>
          <div class="home2-flow-thumb"><img src="${row.image}" alt="${escapeHtml(row.alt)}" width="480" height="320" loading="lazy" /></div>
          <p class="home2-flow-eyebrow">${escapeHtml(row.eyebrow || 'operando.app')}</p>
  ...
  <section class="home2-metrics" aria-labelledby="live-metrics-title">
    ${publishedMetrics.length ? `
    <div class="home2-marquee marketing-live-metrics-grid">
      ${publishedMetrics.map((metric, index) => `${index ? '<span class="home2-marquee-sep" aria-hidden="true">•</span>' : ''}<div class="home2-marquee-item"><strong class="marketing-counter" data-counter-value="${Number(metric.value)}" data-counter-prefix="${escapeHtml(metric.prefix || '')}" data-counter-suffix="${escapeHtml(metric.suffix || '')}" data-counter-format="${escapeHtml(metric.format || 'integer')}">${escapeHtml(metric.prefix || '')}${metric.format === 'millions' ? Math.round(Number(metric.value) / 1000000).toLocaleString('es-AR') : Number(metric.value).toLocaleString('es-AR')}${escapeHtml(metric.suffix || '')}</strong><span>${escapeHtml(metric.label)}</span></div>`).join('')}
    </div>` : ''}
    <div class="home2-verticals">
      <p class="home2-kicker-plain">Diseñado para crecer con tu rubro</p>
      <h2 id="live-metrics-title">Funciona para comercios de <em>todos los rubros</em></h2>
      <ul class="home2-pill-list" aria-label="Rubros incluidos">${verticals.map((vertical) => `<li>${escapeHtml(vertical)}</li>`).join('')}</ul>
      <p class="home2-fineprint">Los rubros son ejemplos: operando.app se adapta a la operación de cualquier comercio que necesite vender, cobrar, controlar stock y trabajar con su equipo.</p>
    </div>
  </section>
  <section class="home2-support" aria-labelledby="support-title">
    <div class="home2-support-copy">
      <p class="home2-kicker-plain">Soporte 24/7</p>
      ...
      <ul class="home2-support-list">
        <li><b>24/7</b><span>Para acompañar la operación, incluso fuera del horario de mostrador.</span></li>
        <li><b>WhatsApp directo</b><span>Escribinos como le escribirías a alguien de tu equipo.</span></li>
  ...
  <section class="home2-control" aria-labelledby="control-title">
    <div class="home2-section-head">
      <p class="home2-kicker-plain">Recorrido de control</p>
      <h2 id="control-title">Seis controles para operar con más contexto</h2>
      <p>Lo esencial para vender, controlar y revisar sin sumar pasos en el mostrador.</p>
    </div>
    <div class="home2-accordion">
      ${controlStories.map((story, index) => `
        <details class="home2-accordion-item" ${index === 0 ? 'open' : ''}>
          <summary>
            <span class="home2-accordion-num">${escapeHtml(story.number)}</span>
            <span class="home2-accordion-title">${escapeHtml(story.eyebrow)}</span>
            <span class="home2-accordion-caret" aria-hidden="true"></span>
          </summary>
          <div class="home2-accordion-content">
    ...
    <p class="home2-control-more"><a href="/funciones/" data-analytics="home_control_more">Ver todas las funciones</a></p>
  </section>
  <section class="home2-close" aria-labelledby="close-title">
    <div class="home2-close-card">
      <p class="home2-kicker-plain">Empezá hoy</p>
      <h2 id="close-title">Probá operando.app en tu propio negocio</h2>
      <p>Creá tu cuenta en minutos y conocé la herramienta trabajando con tus productos y tus ventas.</p>
      <div class="home2-close-actions">
        <a class="home2-btn-primary" href="${signupPath}" data-analytics="home_cta_signup">Crear cuenta</a>
        <a class="home2-btn-secondary" href="${loginPath}" data-analytics="home_cta_login">Iniciar sesión</a>
      </div>
    </div>
  </section>
`
...
const marketingStyles = `
  ...
  .marketing-section-intro p:last-child { margin: 0; color: #b9c2ce; line-height: 1.6; }
      ...
      /* ============================================================
         HOME v2 — "Panel de turno" system (structure exclusive to home)
         Every rule below targets home2-* classes that ONLY the home page
         emits (see renderHomeHero / renderHomeExtras). None of the 30+
      ...
      /* --- Shared home2 kicker styles --- */
      body[data-page="home"] .home2-kicker,
      body[data-page="home"] .home2-kicker-plain {
        display: inline-flex;
        align-items: center;
        gap: 9px;
        font-family: var(--font-mono);
      ...
      body[data-page="home"] .home2-section-head { max-width: 640px; margin: 0 auto 40px; text-align: center; }
      body[data-page="home"] .home2-section-head h2 {
      ...
      body[data-page="home"] .home2-section-head p { color: var(--text-dim); }

      /* --- 1. Hero: split panel with a tabbed terminal-style screenshot --- */
      body[data-page="home"] .home2-hero {
        display: grid;
      ...
      body[data-page="home"] .home2-hero-copy h1 {
        font-family: var(--font-display);
      ...
      body[data-page="home"] .home2-lead {
        max-width: 48ch;
        font-size: 1.05rem;
        line-height: 1.6;
        color: var(--text-dim);
      ...
      body[data-page="home"] .home2-hero-actions { display: flex; align-items: center; gap: 22px; flex-wrap: wrap; }
      ...
      body[data-page="home"] .home2-btn-secondary:hover { border-color: var(--red-soft); color: var(--red-soft); }
      body[data-page="home"] .home2-btn-link {
        font-family: var(--font-mono);
        font-size: 0.88rem;
        color: var(--text-dim);
      }
      body[data-page="home"] .home2-btn-link span { color: var(--text); text-decoration: underline; text-decoration-color: var(--line); }
      body[data-page="home"] .home2-btn-link:hover span { color: var(--red-soft); text-decoration-color: var(--red-soft); }

      body[data-page="home"] .home2-hero-panel { display: flex; flex-direction: column; gap: 14px; }
      ...
      body[data-page="home"] .home2-tab-input { position: absolute; opacity: 0; pointer-events: none; }
      body[data-page="home"] .home2-frame-bar {
        display: flex;
        align-items: center;
        gap: 4px;
        padding: 10px 12px;
      ...
      body[data-page="home"] .home2-frame-dot {
      ...
      body[data-page="home"] .home2-frame-tab {
        padding: 6px 12px;
        border-radius: 5px;
        font-family: var(--font-mono);
        font-size: 0.78rem;
      ...
      body[data-page="home"] .home2-frame-tab:hover { color: var(--text); }
      body[data-page="home"] .home2-frame-body { position: relative; }
      body[data-page="home"] .home2-pane { display: none; }
      body[data-page="home"] .home2-pane img { display: block; width: 100%; }
      ...
      body[data-page="home"] #home2-tab-stock:checked ~ .home2-frame-body [data-pane="stock"],
      body[data-page="home"] #home2-tab-caja:checked ~ .home2-frame-body [data-pane="caja"] {
        display: block;
      }
      body[data-page="home"] .home2-statusbar {
        display: grid;
      ...
      body[data-page="home"] .home2-status-item {
        display: flex;
      ...
      body[data-page="home"] .home2-status-item:first-child { border-left: 0; }
      body[data-page="home"] .home2-status-item b {
        font-family: var(--font-mono);
        font-size: 0.66rem;
        font-weight: 500;
        letter-spacing: 0.06em;
      ...
      body[data-page="home"] .home2-status-item span { font-size: 0.82rem; color: var(--text-dim); line-height: 1.35; }

      /* --- 2. Flow strip: horizontal connected steps --- */
      body[data-page="home"] .home2-flow-track {
        position: relative;
        display: grid;
        grid-template-columns: repeat(3, 1fr);
        gap: 28px;
      }
      body[data-page="home"] .home2-flow-track::before {
        content: '';
        position: absolute;
        top: 15px;
        left: 4%;
      ...
      body[data-page="home"] .home2-flow-step { position: relative; z-index: 1; }
      body[data-page="home"] .home2-flow-num {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        width: 30px;
      ...
      body[data-page="home"] .home2-flow-thumb {
        border: 1px solid var(--line);
        border-radius: 8px;
        overflow: hidden;
        margin-bottom: 16px;
      }
      body[data-page="home"] .home2-flow-thumb img { display: block; width: 100%; transition: transform 0.4s ease; }
      body[data-page="home"] .home2-flow-step:hover .home2-flow-thumb img { transform: scale(1.03); }
      body[data-page="home"] .home2-flow-eyebrow {
        font-family: var(--font-mono);
        font-size: 0.72rem;
      ...
      body[data-page="home"] .home2-flow-step h3 {
        font-family: var(--font-display);
        font-size: 1.15rem;
        margin: 0 0 8px;
        color: var(--text);
      }
      body[data-page="home"] .home2-flow-step p { color: var(--text-dim); font-size: 0.92rem; }

      /* --- 3. Metrics marquee + rubro pill wall --- */
      body[data-page="home"] .home2-marquee {
        display: flex;
        flex-wrap: wrap;
        align-items: baseline;
        justify-content: center;
      ...
      body[data-page="home"] .home2-marquee-item { display: inline-flex; align-items: baseline; gap: 8px; }
      body[data-page="home"] .home2-marquee .marketing-counter {
        font-family: var(--font-mono);
        font-weight: 600;
        font-size: 1.3rem;
        color: var(--text);
      }
      body[data-page="home"] .home2-marquee-item span {
        font-family: var(--font-mono);
        font-size: 0.76rem;
        color: var(--text-dim);
        text-transform: uppercase;
      ...
      body[data-page="home"] .home2-marquee-sep { color: var(--line); font-size: 1.1rem; }
      body[data-page="home"] .home2-verticals { max-width: 720px; margin: 0 auto; text-align: center; }
      body[data-page="home"] .home2-verticals h2 {
        font-family: var(--font-display);
        font-size: clamp(1.9rem, 3.2vw, 2.6rem);
        margin: 14px 0 22px;
        color: var(--text);
      }
      body[data-page="home"] .home2-verticals em { color: var(--red-soft); font-style: normal; }
      body[data-page="home"] .home2-pill-list {
        display: flex;
        flex-wrap: wrap;
        justify-content: center;
        gap: 8px;
      ...
      body[data-page="home"] .home2-pill-list li {
        padding: 7px 14px;
        border: 1px solid var(--line);
        border-radius: 999px;
        font-family: var(--font-mono);
      ...
      body[data-page="home"] .home2-fineprint { color: var(--text-dim); font-size: 0.88rem; max-width: 60ch; margin: 0 auto; }

      /* --- 4. Support as a terminal log --- */
      body[data-page="home"] .home2-support {
        display: grid;
        grid-template-columns: 1fr 1fr;
        gap: 48px;
        align-items: start;
      }
      body[data-page="home"] .home2-support-copy h2 {
        font-family: var(--font-display);
        font-size: clamp(1.9rem, 3.2vw, 2.5rem);
        margin: 14px 0 12px;
        color: var(--text);
      }
      body[data-page="home"] .home2-support-copy p { color: var(--text-dim); }
      body[data-page="home"] .home2-support-list { list-style: none; margin: 22px 0 0; padding: 0; display: flex; flex-direction: column; gap: 14px; }
      ...
      body[data-page="home"] .home2-accordion-item { border-bottom: 1px solid var(--line); }
      body[data-page="home"] .home2-accordion-item summary {
        display: flex;
        align-items: center;
        gap: 18px;
        padding: 20px 4px;
      ...
      body[data-page="home"] .home2-accordion-item summary::-webkit-details-marker { display: none; }
      body[data-page="home"] .home2-accordion-num {
        font-family: var(--font-mono);
        font-size: 0.85rem;
        color: var(--red-soft);
        flex: none;
      ...
      body[data-page="home"] .home2-accordion-title {
        font-family: var(--font-display);
        font-size: 1.2rem;
        color: var(--text);
        flex: 1;
      }
      body[data-page="home"] .home2-accordion-caret {
        flex: none;
        width: 22px;
        height: 22px;
        border: 1px solid var(--line);
      ...
      body[data-page="home"] .home2-accordion-caret::before,
      body[data-page="home"] .home2-accordion-caret::after {
        content: '';
        position: absolute;
        top: 50%; left: 50%;
      ...
      body[data-page="home"] .home2-accordion-caret::before { width: 9px; height: 1px; }
      body[data-page="home"] .home2-accordion-caret::after { width: 1px; height: 9px; transition: transform 0.2s ease, opacity 0.2s ease; }
      body[data-page="home"] .home2-accordion-item[open] .home2-accordion-caret::after { opacity: 0; }
      body[data-page="home"] .home2-accordion-content { padding: 0 4px 26px 46px; max-width: 68ch; }
      body[data-page="home"] .home2-accordion-content h3 {
        font-family: var(--font-display);
        font-size: 1.3rem;
        margin: 0 0 10px;
        color: var(--text);
      }
      body[data-page="home"] .home2-accordion-content p { color: var(--text-dim); margin: 0 0 14px; }
      body[data-page="home"] .home2-accordion-content ul { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 8px; }
      body[data-page="home"] .home2-accordion-content li {
        padding-left: 16px;
      ...
      body[data-page="home"] .home2-control-more { text-align: center; margin-top: 32px; }
      body[data-page="home"] .home2-control-more a { font-family: var(--font-mono); font-size: 0.88rem; color: var(--red-soft); text-decoration: underline; text-decoration-color: var(--line); }
      body[data-page="home"] .home2-control-more a:hover { text-decoration-color: var(--red-soft); }

      /* --- 6. Close: "cerrar turno" centered card --- */
      body[data-page="home"] .home2-close-card {
        max-width: 560px;
        margin: 0 auto;
        padding: 44px 40px;
        text-align: center;
      ...
      body[data-page="home"] .home2-close-card h2 {
        font-family: var(--font-display);
        font-size: clamp(1.8rem, 3vw, 2.3rem);
        margin: 14px 0 10px;
        color: var(--text);
      }
      body[data-page="home"] .home2-close-card p { color: var(--text-dim); margin: 0 0 26px; }
      body[data-page="home"] .home2-close-actions { display: flex; align-items: center; justify-content: center; gap: 16px; flex-wrap: wrap; }

      /* --- Reveal-on-scroll targets (JS toggles is-revealed) --- */
      body[data-page="home"] .home2-flow,
      body[data-page="home"] .home2-metrics,
      body[data-page="home"] .home2-support,
      body[data-page="home"] .home2-control,
      body[data-page="home"] .home2-close {
        position: relative;
      }
      @media (prefers-reduced-motion: no-preference) {
        body[data-page="home"] .home2-flow,
      ...
      @media (max-width: 860px) {
        body[data-page="home"] .home2-hero { grid-template-columns: 1fr; }
        body[data-page="home"] .home2-flow-track { grid-template-columns: 1fr; }
        body[data-page="home"] .home2-flow-track::before { display: none; }
        body[data-page="home"] .home2-support { grid-template-columns: 1fr; }
        body[data-page="home"] .home2-close-card { padding: 32px 22px; }
      }
`
    ...
    <script>
      ...
      if (document.body.dataset.page === 'home') {
        ...
        if (!window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
          const revealTargets = document.querySelectorAll('.home2-flow, .home2-metrics, .home2-support, .home2-control, .home2-close');
