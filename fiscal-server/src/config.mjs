...
export const config = {
  ...
  discord: {
    enabled: process.env.OPERANDO_CONTROL_DISCORD_ENABLED === 'true',
    generalWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_GENERAL_WEBHOOK_URL || '').trim(),
    logsWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_LOGS_WEBHOOK_URL || '').trim(),
    alertasWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_ALERTAS_WEBHOOK_URL || '').trim(),
    arcaWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_ARCA_WEBHOOK_URL || '').trim(),
    securityWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_SEGURIDAD_WEBHOOK_URL || '').trim(),
    gcpRunWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_GCP_RUN_WEBHOOK_URL || '').trim(),
    backupsWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_BACKUPS_WEBHOOK_URL || '').trim(),
    deploysWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_DEPLOYS_WEBHOOK_URL || '').trim(),
    summaryWebhookUrl: String(process.env.OPERANDO_CONTROL_DISCORD_RESUMEN_WEBHOOK_URL || '').trim(),
  },
  telegram: {
    enabled: process.env.OPERANDO_CONTROL_TELEGRAM_ENABLED === 'true',
    botToken: String(process.env.OPERANDO_CONTROL_TELEGRAM_BOT_TOKEN || '').trim(),
    chatId: String(process.env.OPERANDO_CONTROL_TELEGRAM_CHAT_ID || '').trim(),
  },
